...
| 							Initialization Functions                           |
	...
	while(1){
		...
		/*
		 * this case check that the control ECU received first password entered by user
		...
		case PASSWORD_ONE_IS_RESIVED_Q:
			...
			/*
			 * this case check that the control ECU received second password entered by user
			 * and answer by a flag
			 *
			 */
		...
		case PASSWORD_TWO_IS_RESIVED_Q:
			...
			/*
			 * this case check that two password are equals and send answer to HMI as a flag
			 * and send this password to EEPROM to save it as loop because they are array
			 * of bytes, and if not equals arrays send to HMI that two are not equal.
			 *
			 */
		...
		case ASK_UART_ABOUT_MATCHING_PASSWORDS:
			...
			/*
			 * this case used to check the password that user entered equal to the
			 * password saved in it before, if matched send to HMI to able to the user
			 * to continue if not tell user to enter again.
			 *
			 */
		...
		case SENDED_DATA_MATCHES_EEPROM:
			...
			/*
			 * after check that password that user entered matches that password
			 * stored in EEPROM it will implement what the user choose in this case user
			 * choose to open the door the motor will rotate CW for 15 second and stop for
			 * 3 second then rotate A_CW for 15 second.
			 *
			 */
		...
		case DOOR_IS_OPEN:
			...
			/*
			 * this password check that the user enter the password matches to the password
			 * entered before and stored in EEPROM if true send to HMI to allow to user to
			 * change password, if not matched password not allowed and return to last screen.
			 *
			 */
		...
		case CHANGE_PASSWORD :
			...
			/*
			 * this case for check that the first new password entered by user and
		...
		case CHECK_FIRST_NEW_PASSWORD:
			...
			/*
			 * this case for check that the second new password entered by user and
			 * control ECU received it, then till HMI to move to next instruction
			 *
			 */
		case CHECK_SECONDE_NEW_PASSWORD:
			...
			/*
			 * this password check that the two new password are matched and then store it
			 * in a EEPROM as a password of the system, then till HMI by USART it is true or
			 * not, if not it will ask user to enter again two matches passwords.
			 *
			 */
		...
		case CHECK_TWO_NEW_PASSWORD_ARE_EQUAL :
			...
			/*
			 * this case if the user enter wrong password and take 3 times to enter right password
			 * if the user can not enter the password, HMI tell control ECU through UART to
			 * implement this case and return after 1 minute
			 *
			 */
